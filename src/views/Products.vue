<template>
    ...
    <div class="card-body p-6">
      <div>
        <div class="stat-card bg-blue-500 text-white p-4 rounded-lg text-center">
          ...
          <div role="status" v-if="totalProdutos == 0">
            <p class="stat-value text-2xl" >Carregando ...</p>
          </div>
          <p class="stat-value text-2xl" v-else>{{ totalProdutos }}</p>
        </div>
      </div>
    </div>
    <div class="p-6">
          ...
          <div class="overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr v-for="produto in produtos" :key="produto.id">
                <td
                    class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium text-gray-800"
                >
                  {{ produto.id }}
                </td>
                <td
                ...
                >
                  <p
                      :class="statusBg(produto.status)"
                      class="border rounded text-white p-2"
                  >
                ...
                >
                  <button
                      @click="findProductSearch(produto.id)"
  ...
  <div v-if="showModal" class="fixed inset-0 z-50">
    ...
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
      <div
          class="relative p-4 w-full max-w-2xl bg-white rounded-lg shadow dark:bg-gray-700"
      >
        <div
